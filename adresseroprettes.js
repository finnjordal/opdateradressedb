...
function func(data, input, output, adresser) {
 ...
 // console.log("adresse:"+ util.inspect(adresse));
  ...
  adresser.findOne({"adgangsadresse.id":adgangsadresse.id}, function(err, doc) {
    if (--active <= limit) {
      input.resume();
     //console.log('resume (%s), total: (%s)',active, total);
    }
    if (doc !== null) {
      console.log('adresse findes (%s), total: (%s), err: (%s), doc: (%s)',adgangsadresse.id, total, err, doc);
    }
    else {
      console.log('adresse findes ikke (%s), total: (%s), err: (%s)',adgangsadresse.id, total, err);
      var adresse= {};
      ...
      adresse.adgangsadresse.id= adgangsadresse.id;   //AddressAccessReference
      ...
      adresser.insert(adresse, {safe: true}, function(err, object) {
        if (err) {
          ...
          console.log('adresse indsat (%s)', ++indsat);
        }
      });
    }
  }); 
